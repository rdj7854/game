  ...
  for (var i = 0; i < grid.length; i++) {
    ...
    switch (gridNum) {
      ...
      case 2048:
        ...
        break;
      default:
        grid[i].style.backgroundColor = "#cfcec4";
    }
  }
}
...
function testGameOver() {
  ...
  if (count == 16) {
    ...
    for (var i = 0; i < grid.length; i++) {
      if (
        !howToGo(grid[i], "left", 1, true) &&
        !howToGo(grid[i], "right", 1, true) &&
        !howToGo(grid[i], "up", 1, true) &&
        !howToGo(grid[i], "down", 1, true)
      ) {
        count--;
        if (count == 0) {
          popup("game-over");
